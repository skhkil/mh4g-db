...
for r in rewards:
    if r.get('monster') in names and clean(r.get('item')): reward_by[r['monster']].add(clean(r['item']))
quests_by=collections.defaultdict(list)
for q in quests:
    hay=' '.join(str(q.get(k,'') or '') for k in ['name','nameJa','objective','subObjective','note'])
    hits=[n for n in names if n in hay]; hits=[n for n in hits if not any(n!=m and n in m for m in hits)]
    for n in hits:
...
out=D/'monster_refs'; out.mkdir(exist_ok=True); index={}
for m in mons:
...
(D/'monster_reference_index.json').write_text(json.dumps({'version':'0.7.7-chat4-monster1','items':index},ensure_ascii=False,separators=(',',':')),encoding='utf-8')
